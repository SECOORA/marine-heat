...
for i, stat in stat_all.iterrows():
    ...
    for n in dep:
        ...
        wt_days[txt] = group["mean"].where(group["n_nan"] <= 6)
        
        g = wt_hour.groupby(pd.Grouper(freq="MS"))
        group = g[txt].agg(
            mean="mean",
            n_total="size",
            n_valid="count"
        ...
        group["n_nan"] = group["n_total"] - group["n_valid"]
        wt_mons[txt] = group["mean"].where(group["n_nan"] <= 168)
...
e = time.time()
print("Successfully processed all stations at " + str(round((e - s), 3)) + " seconds")
