...
def map_C03(x_min,x_max,y_min,y_max):
    ...
    ax.set_xticks(np.arange(-360,+361,step=1))
    ax.xaxis.set_minor_locator(MultipleLocator(1))
    ax.set_xticklabels(abs(np.arange(-360,+361,step=1)))
    ax.set_xlim(x_min,x_max)
        
    # y-tick labels and limits
    ax.set_yticks(np.arange(-90,+91,step=1))
    ax.yaxis.set_minor_locator(MultipleLocator(1))
    ax.set_yticklabels(np.arange(-90,+91,step=1))
