...
C1 = ((~stat_all["Reg"].str.strip().str.startswith('41')) & 
      ...
      (~stat_all["Reg"].str.strip().str.startswith('44')))
...
condW = stat_all["Lon"] > -87.5183333
condE = stat_all["Lon"] < -74.0
condN = stat_all["Lat"] < +36.5
condS = stat_all["Lat"] > +22.0

Pt = ( condW & condE & condN & condS )

# get subset of stations
stat_reg = stat_all.loc[Pt].copy().reset_index(drop=True)

# prepare the columns and write them to a CSV file
stat_reg["Reg"] = stat_reg["Reg"].str.rjust(40) + " "
stat_reg["Type"] = stat_reg["Type"].str.ljust(5)
stat_reg["Abr"] = stat_reg["Abr"].str.ljust(17)
stat_reg["Lon"] = stat_reg["Lon"].map("{:+9.3f}".format) + " "
stat_reg["Lat"] = stat_reg["Lat"].map("{:+9.3f}".format) + " "
stat_reg["Org"] = stat_reg["Org"].str.replace("\"","").str.ljust(125)
stat_reg["Name"] = stat_reg["Name"]

stat_reg.to_csv('Docs/List_Redux_NDBC.txt',index=False,sep="|")
