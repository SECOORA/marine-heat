...
for t in ["FACT","NDBC","SWT"]:
    ...
    for i, stat in stat_type.iterrows():
        ...
        # get first and last year of time series
        yr1 = df.index[ 0].to_datetime64().astype('datetime64[Y]')
        yr2 = df.index[-1].to_datetime64().astype('datetime64[Y]') + np.timedelta64(1,'Y')
        ...
        for y in np.arange(yr1,yr2):
            ...
            file2save = folder + stat["Reg"] + "_" + y.astype(str) + ".png"
